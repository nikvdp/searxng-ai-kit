...
def fetch_urls(urls: List[str], *, max_concurrent: int = 5) -> List[Dict[str, Any]]:
    """
...
def get_available_engines() -> Dict[str, Any]:
    ...
    total_engines = len(
        set(engine for engines in categories.values() for engine in engines)
    )

    return {"categories": categories, "total_engines": total_engines}
    ...
    *,
    ...
    if result.get("success"):
        ...
        result["total_results"] = (
            result.get("number_of_results", actual_count) or actual_count
        )
