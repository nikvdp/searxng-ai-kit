...
def main():
    ...
    run_command([
        ...
        "--metadata", str(wheels_dir / "build_metadata.json"),
        "--repo-owner", "local",
        "--repo-name", "development", 
        "--wheel-filename", wheel_file.name
    ], cwd=script_dir)
    ...
    content = content.replace(
        "https://github.com/local/development/releases/download/", 
        "file://"
    ).replace(
        f"searxng-wheels-{wheel_file.name}/",
        str(wheels_dir.absolute()) + "/"
    )
    ...
    print("  3. Run tests: uv run python -c 'import searxng; print(\"Import successful\")'")
